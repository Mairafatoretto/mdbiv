...
Xrho <- model.matrix( ~0 + treat)


link1 <- "log"
link2 <- "identity"

#considering others scenarios (0.2 and 0.8), (-0.8 and 0.2) and (-0.2 and 0.5)
rho1 <- 0.2
rho2 <- 0.5
...
##################################################################
   ...
   #simulation
   a <-  diag(c(Xrho%*%as.numeric(c(rho1,rho2))))
   b <- diag(1,n)
   ...
   res <- y - mu.hat
   V <- diag(as.numeric(diag(res%*%t(res))))
   ...
   deriv_mu2 <- attr(eval(deriv2),"gradient")
   du2 <- 1/deriv_mu2
   Diag.mu <- diag(as.numeric(rbind(du1,du2)))
   
   
   variance_beta <- solve(t(X)%*%X)%*%t(X)%*%solve(dmu)%*%V%*%solve(dmu)%*%X%*%solve(t(X)%*%X)
   ...
   variance_mu <- solve(crossprod(X))%*%t(X)%*%dmu%*%solve(Sigma3)%*%V%*%solve(Sigma3)%*%dmu%*%X%*%solve(crossprod(X))
   
   

   
   
   variance_phi <- diag(diag(as.numeric(solve(crossprod(Xphi,Xphi))%*%t(Xphi)%*%(diag(solve(Diag.mu%*%Diag.mu))*((y-mu.hat)+as.matrix(diag(res%*%t(res)))))))%*%variance_mu%*%diag(as.numeric(solve(crossprod(Xphi,Xphi))%*%t(Xphi)%*%(diag(solve(Diag.mu%*%Diag.mu))*((y-mu.hat)+as.matrix(diag(res%*%t(res))))))))
   ...
   stder_beta <- sqrt(diag(variance_beta))
...
rho1_est<-  as.data.frame(do.call("rbind", coefs2$rho))[,1]
rho2_est<-  as.data.frame(do.call("rbind", coefs2$rho))[,2]
error_rho1 <-  as.data.frame(do.call("rbind", coefs2$error_rho))[,1]
error_rho2 <-  as.data.frame(do.call("rbind", coefs2$error_rho))[,2]
...
int_rho1 <- cbind(lower = rho1_est -1.96*error_rho1,
                 upper =  rho1_est+1.96*error_rho1)

int_rho2 <- cbind(lower = rho2_est -1.96*error_rho2,
                  upper =  rho2_est+1.96*error_rho2)
...
for(i in 1:1000){
   ...
   if (int_rho1[i,1]<=rho1 & int_rho1[i,2]>=rho1)
   {
      rho1_c[i]<- 1
   }
   else{ 
   ...
   if (int_rho2[i,1]<=rho2 & int_rho2[i,2]>=rho2)
   {
      rho2_c[i]<- 1
   }
   else{ 
...
table <- cbind(c(
   #bias
   round(mean(Beta11)-2,3),
   round(mean(Beta12)-3,3),
   round(mean(Beta21)-10,3),
   round(mean(Beta22)-300,3),
   round(mean(phi11)-0.5,3),
   round(mean(phi12)-2,3),
   round(mean(phi21)-1,3),
   round(mean(phi22)-10,3),
   round(mean(rho1_est)-rho1,3),
   round(mean(rho2_est)-rho2,3)),
   
   c(#lower limit
      round(mean(error_betasML11),3),
      round(mean(error_betasML12),3),
      round(mean(error_betasML21),3),
      round(mean(error_betasML22),3),
      round(mean(error_phi11),3),
      round(mean(error_phi12),3),
      round(mean(error_phi21),3),
      round(mean(error_phi22),3),
      round(mean(error_rho1),3),
      round(mean(error_rho2),3)),
   ...
   c(#coverage
      ...
      round(sum(phi11_c)/1000,2),
      round(sum(phi12_c)/1000,2),
      round(sum(phi21_c)/1000,2),
      round(sum(phi22_c)/1000,2),
      round(sum(rho1_c)/1000,2),
      round(sum(rho2_c)/1000,2)))


#saving results
save.image(paste(n,rho1,rho2,sep = "_",".RData"))
